...
# 		lines = self.final_vals_to_lines(final_vals, level, html_formating)
	...
	def _get_html(self):
		...
		if context.get('active_id') and context.get('active_model') == 'account.open.chart':
			...
			rcontext['target_move'] = self._selection_to_str('target_move',wiz_id)
			rcontext['display_account'] = self._selection_to_str('display_account',wiz_id)
			rcontext['report_type'] = self._selection_to_str('report_type',wiz_id)
